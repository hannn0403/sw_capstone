...
parser.add_argument('--load_dir', default='./TABS_model_epoch_9_val_loss_0.1235', type=str)
...
def get_loss(model, criterion, mri_images, targets, mode):
    ...
    # Gen model outputs
    # 모델에 input 값(mri_images)를 입력하여 output을 생성
    ...
    brain_map = (input_squeezed > -1).float() # 1번째 차원을 줄인다.
    stacked_brain_map = torch.stack([brain_map, brain_map, brain_map], dim=1) # brain mat을
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('TABS_model_epoch_255_val_loss_0.0311.pth', map_location=torch.device(args.gpu))
    ...
    with open(log_name, "a") as log_file:
        ...
        log_file.write('\n\n')
        log_file.write('Pearson correlation')
        log_file.write('%s\n\n' % overall_pcorr)
        log_file.write('SD Correlation')
        log_file.write('%s\n\n' % overall_scorr)
        log_file.write('MSE')
        log_file.write('%s\n\n' % overall_mse)
        log_file.write('DECE')
